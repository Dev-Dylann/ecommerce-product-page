...
const ProductInfo = ({product}: ProductInfoProps) => {
  ...
  return (
    <section className='p-5 flex flex-col gap-2 sm:p-7 md:p-10'>
      ...
      <div className='flex gap-4 items-center pt-3'>
        ...
        {product.discount !== undefined && (
          <div className='flex items-center justify-between grow'>
            ...
            <span className='font-bold text-blue-gray line-through sm:text-lg'>${product.price.toFixed(2)}</span>
          </div>
        )}
      </div>

      <div className='bg-blue-lightGray px-5 py-4 mt-3 rounded-md flex justify-between items-center sm:px-7 md:px-10'>
        <button disabled={productCount === 0} onClick={() => setProductCount(prev => prev - 1)} className='p-2'>
          <img src={minusIcon} alt="Decrease" />
        </button>

        <span className='font-bold sm:text-lg'>{productCount}</span>
      ...
      <button disabled={productCount < 1} className='bg-orange shadow-xl shadow-orange-pale rounded-md py-4 flex justify-center gap-4 text-white font-bold mt-1 sm:text-lg' onClick={() => addToCart(product, productCount, price)}>
